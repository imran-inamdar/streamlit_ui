...
def generate_credit_profile(number, random_seed=200):
	fake = Faker("en_US")
	custname = []
	contact=[]
	creditpro = []
	...
	for _ in range(number):
		...
		amount.append(np.random.randint(100,50000))
		cred_sec_code.append(fake.credit_card_security_code())
		cred_expire.append(fake.credit_card_expire())
		zipcode.append(fake.zipcode())

	df = pd.DataFrame(zip(custname,contact,creditpro, cardnum,amount,cred_sec_code, cred_expire, zipcode),
					  columns=['Customer Name','Contact Number','credit card provider', 'Credit card number', 'Transaction amount','Security Code', 'Expiry Date', 'zipcode'])
	return df
...
def main():
	...
	menu = ["Simple","Customize","Credit card"]
	...
	if choice == "Simple":
		...
		# profile_fields = st.sidebar.multiselect("Fields", profile_options_list)
